...
class PdfViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun addText(
        ...
        val x =
            (box.width * xFraction)
                .coerceIn(8f, box.width - 20f)

        val y =
            (box.height * (1f - yFraction))
                .coerceIn(20f, box.height - 20f)
    ...
    fun addSignature(
        ...
        val centerYFromTop =
            box.height * yFraction

        val x =
            (centerX - signatureWidth / 2f)
                .coerceIn(
                    0f,
                    box.width - signatureWidth
                )
    ...
    fun mergePdfs(
        ...
        workingFile?.let { current ->
            ...
            val copy =
                File(
                    context.cacheDir,
                    "merge_base_${System.nanoTime()}.pdf"
                )
        ...
        uris.forEachIndexed { index, uri ->
            ...
            val temp =
                File(
                    context.cacheDir,
                    "merge_${index}_${System.nanoTime()}.pdf"
                )
            ...
            resolver.openInputStream(uri)?.use { input ->
                ...
                "Eine ausgewählte PDF konnte nicht gelesen werden."
            )
        ...
        if (sourceFiles.isEmpty()) {
            error(
                "Keine PDF zum Zusammenführen ausgewählt."
            )
        }
                ...
                val export =
                    withContext(
                        ...
                        val out =
                            File(
                                context.cacheDir,
                                "seite_${index + 1}_${System.nanoTime()}.pdf"
                            )
                        ...
                        PDDocument.load(
                            ...
                            if (pages.isEmpty()) {
                                error(
                                    "Seite konnte nicht exportiert werden."
                                )
                            }
    ...
    fun undo() {
        ...
        runTask(
            "Letzte Änderung wird rückgängig gemacht …"
        ) {
